# Hi, I'm Ajay

I build practical open-source tools around:
- MCP servers for real-world systems and public APIs
- AI developer workflows and automation
- Browser extensions that improve daily productivity
- Observability, OCR, and Atlassian-integrated tooling
- Many CLI tools for developers(port-view, dep-age, repo-scan, sp-dl etc)

I like shipping tools that are useful on day one, easy to demo, and easy to adopt.

<!---
bhayanak/bhayanak is a ✨ special ✨ repository because its `README.md` (this file) appears on your GitHub profile.
You can click the Preview link to take a look at your changes.
--->

## Featured projects

- [MCP Servers](https://marketplace.visualstudio.com/publishers/bhayanak) - MCP servers vscode plugins, they all have servers also in **npmjs** for any GENAI agent support
- [Python tools](https://pypi.org/user/Bhayanak/) - tools you would love to explore and use or extend.
- [Browser Extensions](https://addons.mozilla.org/en-US/firefox/user/19883423/) - Same available for EDGE browser and all chromium based from github release page (chrome store requires money to upload)

### Some Fun projects
- [Puzzles for kids](https://bhayanak.github.io/puzzler/index.html)
- [Fun Race](https://bhayanak.github.io/velocity-moto/)
- [Fun time pass](https://bhayanak.github.io/escape-it/) 
- [kids calculator](https://bhayanak.github.io/calculator/)


## Support

If one of my tools saved you time, you can say thanks here:

[![Buy Me A Chai](https://buymeachai.ezee.li/assets/images/buymeachai-button.png)](https://buymeachai.ezee.li/fazorboy)
